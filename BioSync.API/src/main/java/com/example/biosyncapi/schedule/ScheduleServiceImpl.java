...
public class ScheduleServiceImpl implements ScheduleService {
  ...
  {
    return scheduleRepository.findConflictingSchedules(scheduleDate, startTime,
        endTime, laboratory);
  }
  ...
  public List<Schedule> createSchedule(Schedule schedule) {
    ...
    List<User> students =
        userRepository.findBySectionId(schedule.getSection().getId());
    ...
    if (schedule.getRecurrence() == Recurrence.NONE) {
      ...
      for (User student : students) {
        ScheduleStudent scheduleStudent =
            new ScheduleStudent(schedule, student);
    ...
    startDate.setTime(schedule.getScheduleDate());
    ...
    while (startDate.before(endDate) || startDate.equals(endDate)) {
      for (int index = 0; index < schedule.getRecurrenceDays().size(); index++)
      {
        ...
        while (startDate.get(Calendar.DAY_OF_WEEK) != getCalendarDayOfWeek(
            day)) {
        ...
        for (User student : students) {
          ScheduleStudent scheduleStudent =
              new ScheduleStudent(newSchedule, student);
          scheduleStudents.add(scheduleStudent);
        }
  ...
  private static Schedule setNewSchedule(
      Schedule schedule,
      Calendar startDate)
  {
  ...
  private static final Logger logger =
      Logger.getLogger(ScheduleServiceImpl.class.getName());
    ...
    if (schedule.getRecurrence() != Recurrence.NONE) {
      ...
      for (Schedule relatedSchedule : relatedSchedules) {
        ...
        LocalDate relatedDate =
            convertToLocalDate(relatedSchedule.getScheduleDate());
        ...
        logger.info("Related Date: " + relatedSchedule.getScheduleDate()
            + "Day: " + relatedSchedule.getScheduleDate().toLocalDate()
            .getDayOfWeek());

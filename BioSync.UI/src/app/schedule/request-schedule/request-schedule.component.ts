...
@Component({
  ...
  styleUrls: ['./request-schedule.component.css', '../add-schedule/add-schedule.component.css']
})
export class RequestScheduleComponent  implements OnInit {
  @Input() isRequestOneSchedule!: boolean;
  @Input() isRequestWeeklySchedule!: boolean;
  ...
  initForm(): void{
    this.scheduleForm = this.formBuilder.group({
        ...
        recurrence: [this.isRequestWeeklySchedule ? 'WEEKLY' : 'NONE', [Validators.required]],
    ...
    this.scheduleForm.valueChanges
      .pipe(
        ...
        switchMap(formValues => {
          if (this.isRequestWeeklySchedule && formValues.schoolYear && formValues.semester) {
  ...
  cancelOrAddRequestSchedule(): void {
  ...
  openSomethingWentWrong(){
    ...
    ref.afterClosed().subscribe({
      next: () => {
        this.cancelOrAddRequestSchedule();
      }
    })
  }
